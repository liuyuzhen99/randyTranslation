...
def main() -> int:
    ...
    if session_factory is None:
        raise RuntimeError("DATABASE_URL is required for backend readiness drill.")
    ...
    if non_empty:
        raise RuntimeError(f"backend readiness drill requires empty queues. Non-empty queues: {non_empty}")
    ...
    try:
        ...
        _purge_pipeline_queues(rabbitmq_url, topology)
    result = {
...
def _run_cos_drill(settings) -> dict:
    ...
    drill_id = f"backend-readiness-cos-{uuid4().hex[:12]}"
    workspace = storage.prepare_task_workspace(drill_id)
    source_path = Path(storage.resolve_temp_file(drill_id, "backend-readiness-drill.txt"))
    source_path.write_text("operational cos drill", encoding="utf-8")

    artifact = storage.upload_artifact(
        task_id=drill_id,
        local_path=str(source_path),
        artifact_type="backend_readiness_drill",
        content_type="text/plain",
    )
    downloaded_path = Path(storage.resolve_temp_file(drill_id, "backend-readiness-drill-downloaded.txt"))
    ...
    return {
        ...
        "uploaded": True,
        "downloaded_matches": downloaded == "operational cos drill",
        "deleted": True,
    }


def _purge_pipeline_queues(rabbitmq_url: str, topology: PipelineQueueTopology) -> None:
    import pika
...
def _run_backlog_drill(
    *,
    ...
    jobs = [
        Job(job_id=f"backend-readiness-backlog-{drill_id}-{index}", song_name=f"Backend Readiness Backlog {index}")
        for index in range(3)
    ]
    for job in jobs:
        job_repository.create(job)
        message = command_service.enqueue_first_stage(job, trace_id=f"backend-readiness-backlog-{drill_id}")
...
def _run_dlq_replay_drill(
    *,
    ...
    drill_id = uuid4().hex[:12]
    job = Job(job_id=f"backend-readiness-dlq-{drill_id}", song_name="Backend Readiness DLQ Replay")
    job_repository.create(job)
    first_message = PipelineStageMessage.build(
        ...
        trace_id=f"backend-readiness-dlq-{drill_id}",
        max_attempts=1,
    ...
    failing_worker = PipelineStageWorker(
        ...
        execution_repository=execution_repository,
        command_service=command_service,
        handlers={StageType.AUDIT: lambda _message: (_ for _ in ()).throw(RuntimeError("backend readiness drill failure"))},
    )
