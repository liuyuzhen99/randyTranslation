...
def init_db(db_name):
    ...
    conn.commit()
    conn.close()
    print(f"✅ 数据库初始化完成：{db_name} (包含 artists & videos 表)")
...
class DatabaseConsumer(threading.Thread):
    ...
    def run(self):
        ...
        cursor = conn.cursor()
        print("🗄️ 数据库写入守护线程已启动...")
        
        while True:
            task = self.task_queue.get()
            if task is None: break # 收到 None 则退出
            print(f"📥 消费者收到任务: {task[0] if task else 'None'}") # 打印收到的任务名
            action, data = task
            try:
...
def job_sync_spotify(q):
    print(f"🚀 [{datetime.now()}] 开始同步 Spotify 关注列表...")
    # 这里接入你的 Spotify API 函数
    followed_list = get_all_followed_artists()
    print(f"✅ 获取到 {len(followed_list)} 位关注艺人，正在推送到数据库写线程...")
    # mock_data = get_spotify_followed_artists() 
    # followed_list = [{'id': 'id1', 'name': 'J. Cole'}, {'id': 'id2', 'name': 'Kendrick'}]
    q.put(("SYNC_SPOTIFY", followed_list))
    # print("🏁 Spotify 同步任务推送完毕")

# 任务 B: 每周填充 YouTube ID (受控并发)
def job_fill_youtube_ids(db_name, q, batch_size=20):
    print(f"🔍 [{datetime.now()}] 开始增量抓取 YouTube ID...")
    ...
    print(f"🏁 本次抓取任务推送完毕 (共 {len(targets)} 条)")
